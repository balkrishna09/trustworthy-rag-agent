...
A 2×2 factorial experiment (\{Llama~3.3~70B, Qwen~3.5~35B\} × \{all-MiniLM-L6-v2, Snowflake Arctic Embed2\}) reveals that the Trust Index is LLM-dependent: Qwen~3.5~35B's verbose generation style causes systematic false positives, underperforming the naive baseline by 14 percentage points, while LLM selection dominates embedding model choice as the key performance factor. A retrieval depth sensitivity analysis ($K=3$ vs.\ $K=5$) confirms that detection performance is retrieval-depth-invariant: the 40\% recall ceiling is set by attack strategy difficulty, not by the number of documents retrieved.
...
\include{chapters/ch2_theory_background}
\include{chapters/ch3_literature_review}
\include{chapters/ch4_methodology}
\include{chapters/ch5_eval_agent}
...
% ---------- Appendices ----------
\appendix
\include{chapters/appA_parameters}
\include{chapters/appB_results}
\include{chapters/appC_prompts}
\include{chapters/appD_ethics}
\include{chapters/appE_implementation}
