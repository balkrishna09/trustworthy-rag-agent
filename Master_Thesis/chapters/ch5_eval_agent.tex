...
\begin{tikzpicture}[
    ...
    % Three modules
    \node[mod, below=1.0cm of inp, xshift=-3.6cm]  (nli)   {NLI Verifier\\{\footnotesize nli\_verifier.py}};
    \node[mod, below=1.0cm of inp]                  (pois)  {Poison Detector\\{\footnotesize poison\_detector.py}};
    \node[mod, below=1.0cm of inp, xshift=3.6cm]   (cons)  {Consistency\\Estimator};

    % Trust index
    \node[box, fill=orange!15, below=1.0cm of pois, minimum width=3.4cm] (trust) {Trust Index Calculator\\{\footnotesize trust\_index.py}};

    % Output
    \node[out, below=1.0cm of trust, minimum width=4.2cm] (result) {EvaluationResult\\{\footnotesize (score, level, warnings, report)}};
...
\end{tikzpicture}
\caption{Internal architecture of the Evaluation Agent. Three parallel modules feed into the Trust Index Calculator, which produces the final \texttt{EvaluationResult}.}
\label{fig:eval_agent_arch}
\end{figure}
...
\begin{enumerate}
    ...
    \item \textbf{Cross-document consistency:} pairwise contradiction checks among retrieved documents. Contradiction signals are attributed only to the specific document pair involved via scoped signal indices, preventing a single poisoned document from inflating the poison probability of uninvolved clean documents in the same batch.
...
\centering
\begin{tikzpicture}[
    sig/.style={rectangle, draw=black!70, rounded corners=3pt, fill=red!8,
                minimum width=4.0cm, minimum height=0.75cm,
                text centered, align=center, font=\small},
    agg/.style={rectangle, draw=black!70, rounded corners=4pt, fill=orange!20,
    ...
    % Five signal nodes (left column)
    \node[sig] (s1) {1.\ Linguistic Patterns};
    \node[sig, below=0.3cm of s1] (s2) {2.\ Structural Anomalies};
    \node[sig, below=0.3cm of s2] (s3) {3.\ Intra-doc NLI};
    \node[sig, below=0.3cm of s3] (s4) {4.\ Cross-doc NLI};
    \node[sig, below=0.3cm of s4] (s5) {5.\ Semantic Outlier};

    % Aggregator (right)
    \node[agg, right=2.2cm of s3] (agg) {Relevance-Weighted\\Aggregation};

    % Output
    \node[out, right=1.8cm of agg] (prob) {Poison\\Probability $P$};

    % Arrows from signals to aggregator
    \foreach \s in {s1,s2,s3,s4,s5}{
        \draw[arr] (\s.east) -- (agg.west);
    }
    \draw[arr] (agg) -- (prob);
\end{tikzpicture}
\caption{Poison detection pipeline. Five independent signal detectors feed into a relevance-weighted aggregator to produce the final per-document poison probability $P$.}
\label{fig:poison_pipeline}
\end{figure}
...
The weight configuration was examined through an exploratory ablation study reported in
Chapter~\ref{ch6_experiments}, which tested five configurations including equal weights
(0.33, 0.34, 0.33), consistency-heavy, and poison-heavy settings. The ablation shows that
weight choice affects sensitivity on the smaller ablation subset, but it does not establish
a globally optimal configuration. The default weights are therefore treated as the primary
operating point used in the main experiment rather than as a proven optimum.
...
\begin{tikzpicture}[
    ...
    step/.style={rectangle, draw=black!70, rounded corners=3pt,
                 ...
                 text ragged, align=left, font=\small,
                 inner xsep=8pt},
    arr/.style={-Stealth, thick, black!50}
]
    \node[step, fill=blue!12]    (s1) {\textbf{1.}\ Query enters \texttt{query\_with\_evaluation()}};
    \node[step, fill=orange!12, below=0.3cm of s1] (s2) {\textbf{2.}\ Retriever returns docs, scores, embeddings};
    \node[step, fill=orange!12, below=0.3cm of s2] (s3) {\textbf{3.}\ Generator produces response from context};
    \node[step, fill=red!10,    below=0.3cm of s3] (s4) {\textbf{4.}\ Evaluation Agent computes trust score \& diagnostics};
    \node[step, fill=green!12,  below=0.3cm of s4] (s5) {\textbf{5.}\ \texttt{RAGResponse} returned with \texttt{evaluation} field};
    ...
    \draw[arr] (s4) -- (s5);
\end{tikzpicture}
\caption{RAG pipeline integration flow when evaluation mode is enabled via \texttt{query\_with\_evaluation()}.}
