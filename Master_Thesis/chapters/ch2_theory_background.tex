...
\begin{tikzpicture}[
    ...
    \node[box, fill=orange!20,  right=2.0cm of query]    (ret)   {Retriever\\{\footnotesize(FAISS)}};
    \node[box, fill=green!15,   right=2.0cm of ret]      (gen)   {Generator\\{\footnotesize(LLM)}};
    \node[box, fill=gray!15,    right=2.0cm of gen]      (resp)  {Response};
    \node[box, fill=yellow!20,  below=1.1cm of ret]      (kb)    {Knowledge\\Base};

    \draw[arr] (query) -- (ret);
    \draw[arr] (ret)   -- (gen)  node[midway, above, font=\footnotesize\itshape] {top-$k$ docs};
    \draw[arr] (gen)   -- (resp);
    \draw[arr] (kb)    -- (ret)  node[midway, right, font=\footnotesize\itshape] {index};
\end{tikzpicture}
...
\begin{tikzpicture}[
    ...
    box/.style={rectangle, draw=black!70, rounded corners=4pt,
                ...
                text centered, align=center, font=\small},
    arr/.style={-Stealth, thick, black!60},
    darr/.style={-Stealth, thick, dashed, red!70!black}
]
    % Normal pipeline (top row)
    \node[box, fill=blue!15]   (query)    {Query};
    \node[box, fill=orange!20, right=2.0cm of query]   (ret)   {Retriever\\{\footnotesize(FAISS)}};
    \node[box, fill=green!15,  right=2.0cm of ret]     (gen)   {Generator\\{\footnotesize(LLM)}};
    \node[box, fill=red!20,    right=2.0cm of gen]     (resp)  {Manipulated\\Response};

    % Knowledge base (middle)
    \node[box, fill=yellow!15, below=1.1cm of ret]     (kb)    {Knowledge Base\\{\footnotesize (with poison)}};

    % Attacker (bottom)
    \node[box, fill=red!30, draw=red!80!black,
          below=1.1cm of kb]                           (att)   {\textbf{Attacker}};

    % Arrows
    \draw[arr]  (query) -- (ret);
    \draw[arr]  (ret)   -- (gen)  node[midway, above, font=\footnotesize\itshape] {incl.\ poison doc};
    \draw[arr]  (gen)   -- (resp);
    \draw[arr]  (kb)    -- (ret);
    \draw[darr] (att)   -- (kb)   node[midway, right, font=\footnotesize\itshape, text=red!70!black] {injects};
\end{tikzpicture}
